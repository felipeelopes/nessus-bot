...
class UserExperienceService implements ServiceContract
{
    ...
    /**
     * Query and returns the Global Ranking.
     * @return Collection
     */
    public static function queryGlobalRanking(?Carbon $dateLimit = null): array
    {
        $dateLimitQuery = null;

        if ($dateLimit) {
            $dateLimitQuery = sprintf('`nbot_activities`.`created_at` <= "%s" AND',
                $dateLimit->toIso8601String());
        }
        ...
        return DB::select("
            SELECT
                ...
                ROUND(SUM(`experiences`.`value_duration`) / 3600, 1) AS `player_timing`,
                (
    ...
    public function getGlobalRanking(): Collection
    {
        $globalRanking = Cache::rememberForever(__CLASS__ . '@globalRanking', function () {
            return static::queryGlobalRanking();
        });

        return PlayerRanking::fromQuery($globalRanking);
    }
}
